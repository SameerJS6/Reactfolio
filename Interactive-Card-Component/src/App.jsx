...
export default function App() {
  ...
  const handleSubmit = (e) => {
    ...
    year < currentYear ? alert("Expiration Year Should not be less than Current Year i.e. " + year) : '';
  }
  return (
   <main className='grid grid-rows-2 gap-0 md:gap-8 xl:gap-16 lg:grid-cols-2 lg:grid-rows-none lg:items-center min-h-screen lg:px-8 xl:px-16'>
    <div className="fixed -z-10 left-0 ">
      <picture>
        ...
        <img src={bgMobile} alt="Background Image for Mobile" />
      </picture>
    </div>
        ...
        <div className="input-row | flex gap-4">
            <fieldset className=' flex flex-col gap-2 bg-blue-900 border-none p-1 md:p-2 overflow-hidden w-1/2'>
              ...
              <div className='flex justify-around gap-2'>
                ...
                <input className={`rounded-md text-lg p-2 md:px-4 w-1/2 transition-all duration-300 `} value = {month} onChange={handleMonth} type="tel" id="expiration-month" maxLength={2} required pattern='[0-2]{2}' placeholder='MM'/>
                <input className={`rounded-md text-lg p-2 md:px-4 w-1/2 transition-all duration-300 `} value = {year} onChange={handleYear} type="tel" id="expiration-year" maxLength={4} required pattern='[0-9]{4}' placeholder='YY'/>
